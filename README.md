# Senior Software Engineer Portfolio

Static senior engineering portfolio website built with React, TypeScript, Vite, Tailwind CSS, Motion, and Lucide React.

The site is designed for GitHub Pages and intentionally avoids exposing private contact details. LinkedIn is the primary contact path, and GitHub is configured through `src/data/profile.ts`.

## Local Development

```bash
npm install
npm run dev
```

## Quality Checks

```bash
npm run test:content
npm run typecheck
npm run lint
npm run build
```

## GitHub Pages Deployment

The included workflow at `.github/workflows/deploy.yml` deploys `dist` to GitHub Pages on pushes to `main` and supports manual dispatch.

For a user-site repository such as `<github-username>.github.io`, keep the default base path:

```bash
npm run build
```

For a project-site repository, set `VITE_BASE_PATH` before building:

```bash
VITE_BASE_PATH=/your-repo-name/ npm run build
```

## Updating Content

Profile, links, metrics, experience, skills, projects, impact statements, and AI tooling are stored in `src/data`.

- Update LinkedIn or GitHub links in `src/data/profile.ts`.
- Keep all public content privacy-safe.
- Do not add a direct contact address or phone number to rendered UI, metadata, structured data, or downloadable assets.

## Resume Download

The contact section only shows a resume download button when `VITE_RESUME_URL` is set. Use `/resume.pdf` after adding `public/resume.pdf`, or point it at an external resume URL. No resume file is generated by this project.
